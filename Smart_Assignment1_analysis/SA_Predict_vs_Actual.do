...
local datapull_SA = "06062024" // "11062023" //"05102022" //  "27022023" 
local datapull_hist = "02062024"
local min_cases = 4 


/*****************************************************************
	 Smart Assignment: Import and tempsave it
******************************************************************/

*** Import smart assignment data
	use "`path'\Data_Clean\smartassign_cleaned_`datapull_SA'.dta", clear
	
	drop if is_test == "t" // Drop technical testing cases
	keep if acceptance_or_error_status == "accepted" | ///
	acceptance_or_error_status == "rejected" // Keep accepted and rejected cases
	// (no all ineligible or pending cases)

** Tempsave
	keep acceptance_or_error_status id // Keep variables to identify the cases
	tempfile SA
	save `SA'


/*******************************************************************************
	 VALUE ADDED: Use data from up to June 15, 2023. Calculate VA's. As it was
	 done for SA
*******************************************************************************/

*** Import historical data at the date it was used for Smart Assignment
use "`path'\Data_Clean\cases_cleaned_15062023.dta", clear

*** Keep only cases relevant for the VA calculations (as done in SA) 
...
*** VALUE ADDED: Calculate Shrunk and unshrunk
	...
	* Shrunk VA
	vam case_outcome_agreement, teacher(mediator_id) year(appt_year) class(med_year) controls(i.appt_month_year i.casetype i.courttype i.courtstation i.referralmode) tfx_resid(mediator_id) data(merge tv) // Use calendary-years
	bys mediator_id: egen va_shrunk=mean(tv) // Shrunk VA
	
	* Unshrunk VA
	areg case_outcome_agreement i.appt_month_year i.casetype i.courttype i.courtstation i.referralmode, absorb(mediator_id)
...
/*****************************************************************
	 Add Cases Smart Assignment cases to previous VA's
******************************************************************/

*** Add all historical cases
	merge 1:m mediator_id using "`path'\Data_Clean\cases_cleaned_`datapull_hist'"
	drop if _merge == 2
	drop _merge

*** Add all smart assignment cases
	merge 1:1 id using `SA'
	keep if _merge == 3
	
	keep if acceptance_or_error_status == "accepted" // Only accepted cases

/*****************************************************************
	 Add predicted case outcomes
******************************************************************/
